...
class Action {
  ...
  detectAction(bodyParts) {
    if (this.type == "Wave") {
      ...
      let B = {
        x: bodyParts[13].x * this.WIDTH,
        y: bodyParts[13].y * this.HEIGHT
      };
      let C = {
        x: bodyParts[15].x * this.WIDTH,
        y: bodyParts[15].y * this.HEIGHT
      };
    ...
    if (this.type == "Stomp") {
      ...
      let C = {
        x: bodyParts[27].x * this.WIDTH,
        y: bodyParts[27].y * this.HEIGHT
      };
      let AB = Math.sqrt(Math.pow(B.x - A.x, 2) + Math.pow(B.y - A.y, 2));
      let BC = Math.sqrt(Math.pow(B.x - C.x, 2) + Math.pow(B.y - C.y, 2));
      let AC = Math.sqrt(Math.pow(C.x - A.x, 2) + Math.pow(C.y - A.y, 2));
    ...
    if (this.type == "Fist Pump") {
      ...
      let leftHand = {
        x: bodyParts[19].x * 1280,
        y: bodyParts[19].y * 720
      };
      let rightHand = {
        x: bodyParts[20].x * 1280,
        y: bodyParts[20].y * 720
      };
      let leftHandPinky = {
        x: bodyParts[17].x * 1280,
        y: bodyParts[17].y * 720
      };
  ...
  update(x, y, bodyParts) {
    ...
    this.animation.update(x, y); // not show and reset fps count
...
class Animation {
  ...
  add_frame(frame, offset) {
    // hide the frame
    this.hide_frame(frame); // add the new frame

    this.frames.push(frame); // add the first point offset
  ...
  update(x, y) {
    ...
    for (let i = 0; i < this.frames.length; i++) {
      let frame = this.frames[i]; // update frames' positions
    ...
    if (this.fpsCount >= 0) this.hide_frame(this.frames[this.fpsCount]);
...
class Canvas {
  ...
  contours = [];
  firstPointOffset = [];
  emitters = []; // for the action trigger, will be aggregate into the a sperate class

  FPScount = 0;
  WIDTH = 0;
  HEIGHT = 0;
  contourPoints = []; // color = '#FFFFFF'
  ...
  constructor(width, height) {
    ...
    this.stage.stage.on('mousedown touchstart', e => {
      ...
      let pos = this.stage.stage.getPointerPosition(); // if the pos is not null
    ...
    this.stage.stage.on('mouseup touchend', () => {
      ...
      switch (this.mode) {
        ...
        default:
          ...
          this.currentFrame.push(this.currentLine); // reset current line
  ...
  add_frame(bodyParts) {
    // bodyParts are used to calculate the first point offset
    let firstPointOffset = {
      x: 0,
      y: 0
    };
    ...
    firstPointOffset.y = bodyPart.y * this.HEIGHT - this.currentFrame[0].attrs.points[1]; // add the new frame into animation

    this.currentAnimation.add_frame(this.currentFrame, firstPointOffset); // empty the current shape to prepare next frame

    this.currentFrame = []; // next function should either be add_frame
  }
  ...
  finish_animation(mode) {
    ...
    if (mode === "bind") {
      ...
      this.updateList.push(this.currentAnimation); // reset the currentAnimation
  ...
  new_contour(type) {
    this.contours.push(new _contour.default(this.stage, type));
  }

  action_setup(type) {
    this.currentAnimation.finish();
    this.updateList.push(new _action.default(type, this.currentAnimation));
    this.currentAnimation = null;
  }

  new_motion(type) {
    this.updateList.push(new _motion.default(this.bodyPartID[this.bodyPartID.length - 1], type, this.stage));
  } // trailing_setup() {
  ...
  emit_setup() {
    ...
    let newEmitter = new _emitter.default(this.bodyPartID[this.bodyPartID.length - 1], this.emitLine, // give the emitter line
    ...
    this.currentAnimation.hide_all_frame(); // empty the currentFrame
  ...
  select(id, bodyPart) {
    ...
    this.stage.layer.add(highlight); // create green dot for the selected body part

    this.bodyPartHighlights.push(highlight); // create new animation

    this.currentAnimation = new _animation.default(id, this.stage);
  ...
  update(bodyParts, contourPoints) {
    this.updateList.map(obj => {
      // update animation
      if (obj instanceof _animation.default || obj instanceof _emitter.default || obj instanceof _motion.default) {
        let x = bodyParts[obj.bodyPartID].x * this.WIDTH;
        let y = bodyParts[obj.bodyPartID].y * this.HEIGHT;
        obj.update(x, y);
      } else if (obj instanceof _action.default) {
...
},{"./action.js":58,"./animation.js":59,"./contour.js":62,"./emitter.js":63,"./motion.js":64,"./stage.js":66,"konva":39}],61:[function(require,module,exports){
"use strict";
...
let opencvPlane = document.getElementById('opencv'); // color picker
...
window.app = app; // end of buttons' registration

const inputVideo = document.getElementById('input_video');
const videoElement = document.getElementById("input_video");
const canvasElement = document.getElementById("output_canvas");
const canvasCtx = canvasElement.getContext("2d");
const cvOutput = document.getElementById("canvasOutput"); // get canvas output, opencv would draw on this element

let output = document.getElementById("canvasOutput"); // we are only tracking one element, faking the object as a body
...
let high;
let lastX;
let lastY; // add listener: the handler would set clicked color for opencv mask & filtering

cvOutput.addEventListener("click", e => {
  // get color by coordinate, note that this has to be src[y,x] not src[x,y]
  let color = src.ucharPtr(e.layerY, e.layerX); // threshold for mask

  let range = 15; // get upper bound and lower bound
...
navigator.mediaDevices.getUserMedia({
  ...
  inputVideo.play(); // src = copy of video frame

  src = new cv.Mat(720, 1280, cv.CV_8UC4); // dst = store the processed frame
  ...
  function processVideo() {
    try {
      ...
      cap.read(src); // cv.cvtColor(src, src, cv.COLOR_RGBA2);
      ...
      if (low && high) {
        ...
        let hierarchy = new cv.Mat(); // get remaining contours and hierachy

        cv.findContours(dst, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE); // find the largest are of contours
        ...
        if (maxCnt && maxCnt.data32S) {
          ...
          bodyParts[0].y = Math.floor(sumY / numPoints) / 720; // add new select
          ...
          app.canvas.update(bodyParts, maxCnt.data32S); // let toDraw = new cv.MatVector();
      ...
      cv.imshow("canvasOutput", src); // schedule the next one.
...
},{"./canvas.js":60}],62:[function(require,module,exports){
"use strict";
...
class Contour {
  ...
  update(contourPoints) {
    if (this.type == "line around") {
      let newPoints = this.contourLine.attrs.points; // initialize the line
      ...
      if (newPoints.length == 0) {
        ...
        // find the closet on the contour, add the next, pop the last
        let theClosetPoint = 0;
        let headPoint = {
          x: newPoints[newPoints.length - 2],
          y: newPoints[newPoints.length - 1]
        };
        ...
        theClosetPoint = (theClosetPoint + 4) % contourPoints.length; // delete the trailing point
...
class Emitter {
  ...
  constructor(id, line, animation, stage, offset) {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      this.particles.push(new _particle.default(this.particleAnimation, this.stage, {
  ...
  update(x, y) {
    ...
    let offsetY = Math.floor(y - this.firstPointOffset.y - this.emitLine.attrs.points[1]); // console.log(offsetX, offsetY)
...
},{"./particle.js":65}],64:[function(require,module,exports){
"use strict";
...
class Particle {
  ...
  update(x, y) {
    ...
    this.position.x += x;
    this.position.y += y; //  get the first frame, then the first line

    console.log(this.frames);
    let offsetX = this.position.x - this.frames[0][0].attrs.points[0];
    let offsetY = this.position.y - this.frames[0][0].attrs.points[1]; // if out of the screen, reset on the respawning point
...
},{"konva":39,"lodash":57}],66:[function(require,module,exports){
"use strict";
